  ...
  return (
    <>
      {rows.map((row, rowIdx) => (
        <div
          key={rowIdx}
          className="mt-10 grid grid-cols-1 gap-10 first:mt-0 md:grid-cols-[120px_repeat(4,1fr)] md:gap-8"
        >
    ...
    <div>
      ...
      <ul className="flex flex-col gap-2">
        {items.map((it) => (
          <li
            key={it}
            className="font-serif text-[clamp(18px,2vw,22px)] leading-[1.15] tracking-[var(--tracking-tight)] text-bone"
          >
            {it}
          </li>
        ))}
